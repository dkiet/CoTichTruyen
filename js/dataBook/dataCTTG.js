async function getBooks(myAge) {
  try {
  		...
  		var link = "https://dry-savannah-56936.herokuapp.com/api/books/co-tich-the-gioi?page=1&size=5";
		...
		bookImg.onclick = onBookClickBook2;
		bookName.innerHTML = book[i].title.link("story.html?co-tich-viet-nam/" + book[i].id);

		bookDiv.appendChild(bookImg);
		bookDiv.appendChild(bookName);

		document.getElementById("bookIntro2").appendChild(bookDiv);
		}
	} catch (error) {
  		console.error(error);
	}
...
function onBookClickBook2(e) {
	var targetEl = e.target;
	window.location.href =  "story.html?co-tich-the-gioi/" + targetEl.bookID;
}
